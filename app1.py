...
def context_click_when_enabled(driver, anchor, rel=None, label=None,
    ...
    try:
        ...
        if item and _is_enabled_vakata_item(driver, item):
            ...
            # thử dọn jconfirm rồi thử lại 1 lần
            try:
                if quick_ack_any_popup(driver, root_el=modal):
                    if logger: logger("   (Bắt gặp jconfirm → Đã bấm 'Đồng ý'. Thử lại thao tác menu.)")
                    _hard_close_all_context_menus(driver)
                    _ensure_node_selected(driver, anchor)
                    _open_context_menu(driver, anchor)
                    menu = driver.find_element(By.XPATH,
                    ...
                    if item and _is_enabled_vakata_item(driver, item):
                        try: item.click()
                        except Exception: driver.execute_script("arguments[0].click();", item)
                        return True
            except Exception:
                pass
...
def dismiss_jconfirm(driver, timeout=2.0):
    ...
    while time.time() < end:
        try:
            # Bắt các container jconfirm còn hiển thị
            ...
            # Trong mỗi box, tìm nút Đồng ý (tiếng Việt hoặc bất kỳ nút .btn)
            for box in visible:
                try:
                    # Ưu tiên nút có text 'Đồng ý'
                    try:
                        btn = box.find_element(By.XPATH, ".//button[normalize-space()='Đồng ý']")
                    except NoSuchElementException:
                        # fallback: nút đầu tiên trong cụm nút
                    ...
                    # chờ box biến mất
                    try:
                        WebDriverWait(driver, 2).until(EC.staleness_of(box))
                    except Exception:
                        pass
                    clicked = True
                except Exception:
    ...
    try:
        ...
        if phase_key == "DA_DUYET_KHONG_XL":
            ...
            try:
                ...
                if not ok_boduyet:
                    if quick_ack_any_popup(driver, root_el=modal):
                        logger.log("     ↻ Bắt gặp jconfirm ở B0 → đã bấm 'Đồng ý', thử lại B0.")
                        ok_boduyet = context_click_when_enabled(
                            driver, anchor_vh0,
                            rel=None, label="Bỏ duyệt",  # menu có id=btnBoDuyetTTTD, text = 'Bỏ duyệt'
                            logger=logger.log, modal=modal
        ...
        if not ok_tc and quick_ack_any_popup(driver, root_el=modal):
            logger.log("     ↻ Bắt gặp jconfirm ở B1 → thử lại.")
            ok_tc = context_click_when_enabled(
                driver, anchor_tc,
                rel=2, label="Thêm vào dữ liệu vận hành",
                logger=logger.log, modal=modal
        ...
        ok_vh = context_click_when_enabled(...)
        if not ok_vh and quick_ack_any_popup(driver, root_el=modal):
            logger.log("     ↻ Bắt gặp jconfirm ở B2 → thử lại.")
            ok_vh = context_click_when_enabled(
                driver, anchor_vh,
                rel=4, label="Cập nhật lịch sử tất cả",
                logger=logger.log, modal=modal
...
def resubmit_current_search_if_present(driver, wait, logger=None):
    """
    KHÔNG mở 'Tìm kiếm mở rộng'. Nếu nút 'Tìm kiếm' (btnTimKiemMoRong) có thì bấm để refresh.
    """
    try:
...
def run_phase(driver, wait, logger, phase_name: str, status_value: str, phase_key: str, progress_cb=None):
    ...
    while True:
        ...
        # ===== XỬ LÝ HỒ SƠ =====
        try:
            process_current_record(driver, wait, logger, modal, phase_key=phase_key)
        except Exception as e:
        ...
        if not next_clicked:
            ...
            # Còn hồ sơ: reset theo yêu cầu (đếm lại từ đầu dựa trên tổng mới)
