...
def wait_datatable_reload(driver, table_id="tblDanhSachGoiTinDongBo_info", timeout=5):
...
def wait_status_both(modal, timeout=20):
    """
    Chờ cho đến khi CÙNG xuất hiện 2 trạng thái trong #elementStatus:
      - fa-plus / 'Thêm vào dữ liệu vận hành'
      - fa-refresh / 'Cập nhật lịch sử'
    Trả về (seen_plus, seen_refresh) -> (bool, bool)
    """
    end = time.time() + timeout
    seen_plus = False
    seen_refresh = False

    xp_plus = ".//div[@id='elementStatus']//span[i[contains(@class,'fa-plus')] or contains(normalize-space(.), 'Thêm vào dữ liệu vận hành')]"
    xp_refresh = ".//div[@id='elementStatus']//span[i[contains(@class,'fa-refresh')] or contains(normalize-space(.), 'Cập nhật lịch sử')]"

    while time.time() < end:
        try:
            if not seen_plus:
                spans = modal.find_elements(By.XPATH, xp_plus)
                seen_plus = any(sp.is_displayed() for sp in spans)
            if not seen_refresh:
                spans = modal.find_elements(By.XPATH, xp_refresh)
                seen_refresh = any(sp.is_displayed() for sp in spans)
            if seen_plus and seen_refresh:
                return True, True
        except Exception:
            pass
        time.sleep(0.2)
    ...
    try:
        ...
        if ok_vh:
            ...
            if quick_ack_any_popup(driver, root_el=modal):
                ...
                return "RECOVER"
            logger.log("     ❌ Lỗi B2.")
        seen_plus, seen_refresh = wait_status_both(modal, timeout=20)
        if seen_plus and seen_refresh:
            logger.log("     ✓ Đã thấy cả 2 trạng thái: 'Thêm vào dữ liệu vận hành' và 'Cập nhật lịch sử'.")
        else:
            miss = []
            if not seen_plus: miss.append("Thêm vào dữ liệu vận hành")
            if not seen_refresh: miss.append("Cập nhật lịch sử")
            logger.log(f"     ⚠️ Không thấy đầy đủ badge sau timeout: {', '.join(miss)}. Vẫn tiếp tục.")
        return True
...
def open_filter_and_select_status(driver, wait, status_value: str, logger=None, table_id="formSearchDSDDK"):
    ...
    wait_datatable_reload(driver, table_id="tblDanhSachGoiTinDongBo_info", timeout=5)
...
def recover_close_resubmit_reopen(driver, wait, logger, status_value: str):
    """
    ...
    open_filter_and_select_status(driver, wait, status_value=status_value, logger=logger, table_id="formSearchDSDDK")
    wait_datatable_reload(driver, table_id="tblDanhSachGoiTinDongBo_info", timeout=5)
