...
def open_filter_and_select_chua_xu_ly(driver, wait, table_id="formSearchDSDDK", logger=None):
    """
    Mở 'Tìm kiếm mở rộng' → chờ 1–2s → chọn 'Chưa xử lý' (value='0') → bấm 'Tìm kiếm' → chờ bảng load xong.
    """
    ...
    # --- Bước 2: Chờ form mở hoàn toàn ---
    time.sleep(1.2)  # chờ 1 chút cho animation mở

    # --- Bước 3: Chọn 'Chưa xử lý' ---
    log("🧩 Chọn trạng thái 'Chưa xử lý'…")
    sel = wait.until(EC.presence_of_element_located((By.CSS_SELECTOR, "select[name='trangThai']")))

    try:
        # Ưu tiên native select nếu hiển thị
        driver.execute_script("""
    ...
    try:
        ...
        # Cách 1: chọn theo giá trị
        dropdown.select_by_value("0")
    except Exception:
        pass

    # --- Bước 4: Bấm nút 'Tìm kiếm' ---
...
def run_bot(username, password, code, start_page, logger: UILogger, base_url):
    ...
    try:
        ...
        logger.log("🧰 Mở bộ lọc & áp dụng: Chưa xử lý…")
        open_filter_and_select_chua_xu_ly(driver, wait, table_id="formSearchDSDDK", logger=None)
        time.sleep(1.0)
        # Bấm 'Xử lý đơn'
